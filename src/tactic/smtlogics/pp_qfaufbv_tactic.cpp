...
static tactic * mk_pp_qfaufbv_preamble(ast_manager& m, params_ref const& p) {
    ...
    return
            and_then(
                    ...
                    using_params(mk_solve_eqs_tactic(m), solve_eq_p),  // rainoftime: this can handle some "fast sat" cases
...
static tactic * main_pp(tactic* t) {
    ...
    p.set_bool("sort_store", true);   // taken from z3's default qf_aufbv_tactic
...
tactic * mk_pp_qfaufbv_tactic(ast_manager& m, params_ref const & p, tactic* sat, tactic* smt) {
    ...
    tactic* preamble_st = mk_pp_qfaufbv_preamble(m, p);
...
static tactic * mk_pp_qfaufbv_light_preamble(ast_manager& m, params_ref const& p) {
    params_ref solve_eq_p;
    // conservative guassian elimination.
    solve_eq_p.set_uint("solve_eqs_max_occs", 2);
    ...
    return
            and_then(
                      ...
                      mk_propagate_values_tactic(m),
                      using_params(mk_solve_eqs_tactic(m), solve_eq_p),
                      //mk_solve_eqs_tactic(m),  //TODO: use this or the above one?
                      mk_elim_uncnstr_tactic(m),
                      //mk_bv_size_reduction_tactic(m),
                      using_params(mk_simplify_tactic(m), simp2_p),
                      mk_bvarray2uf_tactic(m, p),        // reduce bv array to bv(with uninterpreted function)
...
tactic * mk_pp_qfaufbv_light_tactic(ast_manager& m, params_ref const & p, tactic* sat, tactic* smt) {
    ...
    tactic* preamble_st = mk_pp_qfaufbv_light_preamble(m, p);
    tactic * st = main_pp(and_then(preamble_st,
                                    cond(mk_is_qfbv_probe(),
...
tactic * mk_pp_qfaufbv_light_tactic(ast_manager & m, params_ref const & p) {
    ...
    return mk_pp_qfaufbv_light_tactic(m, p, new_sat, using_params(mk_smt_tactic(m, p), smt_solver_p));
}

tactic * mk_pp_qfaufbv_tactic(ast_manager & m, params_ref const & p) {
    ...
    return mk_pp_qfaufbv_tactic(m, p, new_sat, using_params(mk_smt_tactic(m, p), smt_solver_p));
}

/*
tactic * mk_pp_qfaufbv_layered_tactic(ast_manager & m, params_ref const & p) {

    return or_else(try_for(mk_pp_qfaufbv_light_tactic(m, p), 15000), mk_pp_qfaufbv_tactic(m, p));
}
*/
