...
Options::Options (Internal * s) : internal (s)
{
  ...
  // First initialize them according to defaults in 'options.h'.
  //
  const char * prev = "";
  size_t i = 0;
# define OPTION(N,V,L,H,O,P,R,D) \
  do { \
    if ((L) > (V)) \
      FATAL ("'" #N "' default '" #V "' " \
        "lower minimum '" #L "' in 'options.h'"); \
    if ((H) < (V)) \
      FATAL ("'" #N "' default '" #V "' " \
        "larger maximum '" #H "' in 'options.h'"); \
    if (strcmp (prev, #N) > 0)  \
      FATAL ("'%s' ordered before '" #N "' in 'options.h'", prev); \
    N = (int)(V); \
    assert (&val (i) == &N); \
