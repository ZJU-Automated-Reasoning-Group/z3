...
// Do include 'internal.h' but try to minimize internal dependencies.

#include "internal.h"
#include "cad-signal.h"           // Separate, only need for apps.
...
// the class 'Solver' is used (defined in 'cadical.h').  It further uses
      ...
      if (lit) cube.push_back (lit);
        ...
        if (!quiet) {
          if (reporting) {
        	 ...
        	 snprintf (buffer, 160, "solving cube %zu / %zu %.0f%%",
...
void App::init () {
  ...
  // 'options.h' related to 'reportdefault' for details.
